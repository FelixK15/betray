...
#include "betray_android_native_app_glue.h"
...
{
	printf( "betray_create_directory_tree(%s, %s)", pBaseDir, pDirTree );
	...
	for( size_t dirTreeIndex = 0u; dirTreeIndex < dirTreeLength; ++dirTreeIndex )
	{
		...
		if( callMkDir )
		{
			printf("mkdir(%s)", tempDirectoryPath);
...
void betray_recreate_asset_directory_tree_recursively( const char* pAssetDirectory, const char* pApplicationDirectory, JNIEnv* env, jobject assetManager, jmethodID listAssets )
{
	...
	for( assetIndex = 0u; assetIndex < assetListLength; ++assetIndex )
	{
		...
		//FK: Check if asset has already been dumped.
		if( access( applicationAssetPath, F_OK ) == 0 )
		{
		...
		if( pAsset == NULL )
		{
			printf( "Creating directory \"%s\" to recreate asset directory tree", applicationAssetPath );
			if( betray_create_directory_tree( pApplicationDirectory, assetPath ) == -1 )
			{
				printf( "Couldn't create directory \"%s\". errno:%d", applicationAssetPath, errno );
				continue;
			}
		...
		{
#ifndef F_ANDROID_EMULATE_FILE_API
			printf("Dumping asset \"%s\"...", assetPath);
			FILE* out = fopen(applicationAssetPath, "wb");
			if( out == NULL )
			{
				printf( "Couldn't dump asset \"%s\", errno:%d", assetPath, errno );
				continue;
			}
...
void* betray_thread_recreate_asset_directory_tree(void* params)
{
	...
	jobject file = (*env)->CallObjectMethod(env, thread_attributes->activity, getCacheDir);
	...
	memcpy( thread_attributes->app_dir, app_dir, app_dir_length );
	thread_attributes->app_dir[ app_dir_length ] = 0;

	betray_recreate_asset_directory_tree_recursively("", app_dir, env, assetManager, listAssets);
...
void betray_set_directory()
{
	...
	thread_attributes->app_dir	= betray_android_state.app_dir;
	...
	f_android_set_global_asset_manager( betray_android_app_state->activity->assetManager );
	chdir(betray_android_state.app_dir);
}
			...
			/*	for(i = 0; i < max_pointer_count; i++)
				{
					...
					{
						input->pointers[i].last_button[0] = input->pointers[betray_android_state.pointers[i]].button[0];
