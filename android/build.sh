...
jdkVersion="17"
keystoreFilePath="$scriptPath/zenith.keystore"
...
packageName="com.shimmer.zenith"
libraryName="zenith.so"
iconName="shimmer"

resourcePath="$scriptPath/res"
assetsPaths=("$scriptPath/../default" "$scriptPath/../theme_alpha" "$scriptPath/../theme_blue" "$scriptPath/../theme_case" "$scriptPath/../theme_color" "$scriptPath/../theme_map" "$scriptPath/../theme_shader" "$scriptPath/../theme_southbank" )
...
forgeSourceFiles=("f_android.c" "f_math.c" "f_matrix_make.c" "f_matrix_operations.c" "f_mem_debug.c" "f_noise.c" "f_path.c" "f_sort.c" "f_text.c" "f_types.c")
imagineSourceFiles=("i_dir.c" "i_library.c" "i_settings.c" "i_shared_memory.c" "i_threads.c" "i_time.c")
...
function generateApk()
{
    ...
    cp -rf "$buildArtifactsPath/libs" "$buildPackagePath/lib"
...
function checkKeystoreFile()
{
    if ! [ -f $keystoreFilePath ]; then
        ...
        if [ "$result" == "n" ]; then
            ...
            if [ "$result" == "y" ]; then
                local keytoolCommand="keytool -keystore $keystoreFilePath -genkey -noprompt -keypass $keystorePassword -storepass $keystorePassword -keyalg RSA -dname \"$keystoreCredentials\""
                echo
...
function generateMakeFiles()
{
    ...
    echo "LOCAL_MODULE := zenith"                                >> $tempAndroidMakeFilePath
    ...
    echo "LOCAL_C_INCLUDES := $includeDir $sourceFileDirectory"  >> $tempAndroidMakeFilePath
    ...
    echo "APP_MODULES := zenith"                    >> $tempAppMakeFilePath
