...
extern void betray_immersive_mode();
...
void android_app_pre_exec_cmd(struct android_app* android_app, int8_t cmd) 
{
    switch (cmd) {
        ...
        case APP_CMD_CONFIG_CHANGED:
            ...
            {
             /*   struct engine* e;
                e = (struct engine*)android_app->userData;
                e->
                if(android_app->window != NULL && ((engine->width != ANativeWindow_getWidth(engine->window)) || (engine->height != ANativeWindow_getHeight(engine->window))))
             */ {
